...
function __init__()
    ...
    if isdefined(Main, :IJulia) && !isdefined(Main, :PROFILEVIEW_USEGTK)
        ...
        @eval begin
            view(data = Profile.fetch(); C = false, lidict = nothing, colorgc = true, fontsize = 12, combine = true, pruned = Set()) = ProfileViewSVG.view(data; C=C, lidict=lidict, colorgc=colorgc, fontsize=fontsize, combine=combine, pruned=pruned)
        end
    else
        eval(Expr(:import, :ProfileViewGtk))
        @eval begin
            view(data = Profile.fetch(); C = false, lidict = nothing, colorgc = true, fontsize = 12, combine = true, pruned = Set()) = ProfileViewGtk.view(data; C=C, lidict=lidict, colorgc=colorgc, fontsize=fontsize, combine=combine, pruned=pruned)
        end
    end
    pop!(LOAD_PATH)
end

function prepare(data; C = false, lidict = nothing, colorgc = true, combine = true, pruned = Set())
...
function svgwrite(filename::AbstractString, data, lidict; C = false, colorgc = true, fontsize = 12, combine = true, pruned = Set())
